...
export class TimesheetStore{
 ...
 private json(data:unknown,status=200,headers?:HeadersInit){return new Response(JSON.stringify(data),{status,headers:{'content-type':'application/json; charset=utf-8','cache-control':'no-store',...(headers||{})}})}
 private async effectiveEmployee(id:string){const base=baseEmployeeById(id);if(!base)return null;const role=await this.state.storage.get<Role>(`role:${id}`);return {...base,role:role||base.role}}
 private async employeeByIdentifier(identifier:string){const x=identifier.trim().toLowerCase();if(!x)return null;const nameMatches=EMPLOYEES.filter(e=>e.name.trim().toLowerCase()===x);if(nameMatches.length===1)return this.effectiveEmployee(nameMatches[0].id);const emailMatches=EMPLOYEES.filter(e=>e.email&&e.email.trim().toLowerCase()===x);if(emailMatches.length===1)return this.effectiveEmployee(emailMatches[0].id);if(emailMatches.length>1)throw new Error('More than one employee uses that email. Please sign in with your full name.');return null}
 private async publicUser(employee:Employee,account?:Account|null):Promise<PublicUser>{const role=await this.state.storage.get<Role>(`role:${employee.id}`)||employee.role;return{employeeId:employee.id,name:employee.name,email:employee.email,role,position:employee.position,mustChangePassword:!!account?.mustChangePassword}}
 ...
 async fetch(request:Request){
  ...
  if(request.method==='POST'&&path==='/auth/bootstrap-admin'){
   const used=await this.state.storage.get<boolean>('bootstrap:done');if(used)return this.json({error:'Office Admin setup has already been completed.'},409);
   const code=this.env.ADMIN_SETUP_CODE||this.env.BOOTSTRAP_ADMIN_CODE;if(!code)return this.json({error:'Office Admin setup code has not been configured in Cloudflare yet.'},503);
   const b=await request.json<Record<string,unknown>>();const identifier=String(b.identifier||'').trim(),provided=String(b.code||''),password=String(b.password||'');
   if(provided!==code)return this.json({error:'Setup code is incorrect.'},403);if(password.length<10)return this.json({error:'Password must be at least 10 characters.'},400);
   let employee:Employee|null=null;try{employee=await this.employeeByIdentifier(identifier)}catch(e){return this.json({error:e instanceof Error?e.message:'Employee not found'},400)}
   if(!employee||!(employee.role==='admin'||employee.position.toLowerCase()==='director'))return this.json({error:'Choose an Office Admin or Director from the staff directory.'},403);
   if(employee.role!=='admin')await this.state.storage.put(`role:${employee.id}`,'admin');
   employee=await this.effectiveEmployee(employee.id);
   if(!employee)return this.json({error:'Employee not found.'},404);
   const account=await this.setPassword(employee.id,password,false);await this.state.storage.put('bootstrap:done',true);const token=randomHex();await this.state.storage.put(`session:${token}`,{employeeId:employee.id,expiresAt:new Date(Date.now()+14*86400000).toISOString()} satisfies Session);
   return this.json({user:await this.publicUser(employee,account)},201,{'set-cookie':this.sessionCookie(token)});
  }
  if(request.method==='GET'&&path==='/auth/bootstrap-status'){const used=!!(await this.state.storage.get<boolean>('bootstrap:done'));return this.json({complete:used,configured:!!(this.env.ADMIN_SETUP_CODE||this.env.BOOTSTRAP_ADMIN_CODE)})}
  if(request.method==='POST'&&path==='/auth/login'){
   const b=await request.json<Record<string,unknown>>();const identifier=String(b.identifier||'').trim(),password=String(b.password||'');let employee:Employee|null=null;try{employee=await this.employeeByIdentifier(identifier)}catch(e){return this.json({error:e instanceof Error?e.message:'Sign in failed'},400)}
   if(!employee)return this.json({error:'Name/email or password is incorrect.'},401);const account=await this.state.storage.get<Account>(`account:${employee.id}`);if(!account?.active)return this.json({error:'Your account has not been activated. Please contact Office Admin.'},403);
   const candidate=await hashPassword(password,account.salt);if(candidate!==account.passwordHash)return this.json({error:'Name/email or password is incorrect.'},401);const token=randomHex();await this.state.storage.put(`session:${token}`,{employeeId:employee.id,expiresAt:new Date(Date.now()+14*86400000).toISOString()} satisfies Session);return this.json({user:await this.publicUser(employee,account)},200,{'set-cookie':this.sessionCookie(token)});
  }
  if(request.method==='POST'&&path==='/auth/change-password'){
   const auth=await this.currentUser(request);if(!auth)return this.json({error:'Please sign in.'},401);const b=await request.json<Record<string,unknown>>();const currentPassword=String(b.currentPassword||''),newPassword=String(b.newPassword||'');if(newPassword.length<10)return this.json({error:'New password must be at least 10 characters.'},400);const current=await hashPassword(currentPassword,auth.account.salt);if(current!==auth.account.passwordHash)return this.json({error:'Current password is incorrect.'},403);const account=await this.setPassword(auth.employee.id,newPassword,false);return this.json({user:await this.publicUser(auth.employee,account)});
  }
  if(request.method==='POST'&&path==='/auth/logout'){const token=parseCookie(request.headers.get('cookie'),'ec_session');if(token)await this.state.storage.delete(`session:${token}`);return this.json({ok:true},200,{'set-cookie':this.sessionCookie('',0)})}
  if(request.method==='GET'&&path==='/auth/me'){const u=await this.currentUser(request);if(!u)return this.json({error:'Not signed in'},401);return this.json({user:await this.publicUser(u.employee,u.account)})}

  const auth=await this.currentUser(request);if(!auth)return this.json({error:'Please sign in'},401);const user=auth.employee;
  ...
  if(request.method==='POST'&&staffMatch){if(user.role!=='admin')return this.json({error:'Office Admin permission required'},403);const employeeId=staffMatch[1],action=staffMatch[2],employee=await this.effectiveEmployee(employeeId);if(!employee)return this.json({error:'Employee not found'},404);if(employee.id===user.id&&action==='deactivate')return this.json({error:'You cannot deactivate your own Office Admin account.'},409);
   if(action==='deactivate'){const account=await this.state.storage.get<Account>(`account:${employeeId}`);if(account){account.active=false;await this.state.storage.put(`account:${employeeId}`,account)}return this.json({ok:true});}
   const temp=temporaryPassword();await this.setPassword(employeeId,temp,true);return this.json({ok:true,tempPassword:temp,employee:{employeeId:employee.id,name:employee.name}});
  }
  ...
  if(request.method==='PATCH'&&path.startsWith('/timesheets/')){
   ...
   else if(action==='pm-approve'){if(user.role!=='pm')return this.json({error:'Project Manager permission required'},403);if(entry.employeeId===user.id)return this.json({error:'You cannot approve your own timesheet'},403);if(entry.status!=='Submitted')return this.json({error:'Only submitted entries can be PM approved'},409);entry.status='PM Approved';entry.pmApprovedBy=user.name}
   else if(action==='admin-approve'){if(user.role!=='admin')return this.json({error:'Office Admin permission required'},403);if(entry.status!=='PM Approved')return this.json({error:'PM approval is required first'},409);entry.status='Admin Approved';entry.adminApprovedBy=user.name;entry.simproPreview=await this.buildSimproPreview(entry);entry.simproStatus=entry.simproPreview.ready?'SAFE MODE: Ready to send':'SAFE MODE: Blocked'}
   else if(action==='reject'){if(user.role==='staff')return this.json({error:'Approval permission required'},403);if(user.role==='pm'&&entry.status!=='Submitted')return this.json({error:'PMs can only reject submitted entries'},409);if(user.role==='admin'&&entry.status!=='PM Approved')return this.json({error:'Office Admin can only reject PM-approved entries'},409);entry.status='Rejected';entry.rejectionReason=reason||'Rejected'}else return this.json({error:'Unknown action'},400);await this.state.storage.put(`entry:${id}`,entry);return this.json({entry});
  }
