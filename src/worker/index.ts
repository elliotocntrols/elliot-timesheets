...
export class TimesheetStore{
 ...
 async fetch(request:Request){
  ...
  if(request.method==='PATCH'&&path.startsWith('/timesheets/')){
   ...
   else if(action==='admin-approve'){if(user.role!=='admin')return this.json({error:'Office Admin permission required'},403);if(entry.status!=='PM Approved')return this.json({error:'PM approval is required first'},409);entry.status='Admin Approved';entry.adminApprovedBy=user.name;entry.simproStatus='Awaiting API connection'}
