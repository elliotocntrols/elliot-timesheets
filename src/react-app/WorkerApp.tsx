...
export default function WorkerApp(){
 ...
 async function reviewEntry(entry:Entry,action:'pm-approve'|'reject'){
  let reason='';
  if(action==='reject')reason=prompt('Reason for rejection?')||'Rejected';
  try{
   await api(`/api/timesheets/${entry.id}`,{method:'PATCH',body:JSON.stringify({action,reason})});
   setMessage(action==='pm-approve'?'Timesheet approved for payroll.':'Timesheet rejected.');
   await load();
 ...
 </form></section>{user.role==='pm'&&<section className="list pm-approval-list"><div className="approval-title"><div><span className="week-label">APPROVALS</span><h2>Timesheets to approve</h2></div><button onClick={load}>Refresh</button></div>{entries.filter(e=>e.status==='Submitted'&&e.employeeId!==user.employeeId).length===0?<p className="muted">No timesheets waiting for approval.</p>:entries.filter(e=>e.status==='Submitted'&&e.employeeId!==user.employeeId).map(e=><div className="row approval-worker-row" key={e.id}><div><b>{e.employee} • {e.date}</b><span>{e.type} • {e.totalHours.toFixed(2)} hrs {e.jobNumber?`• Job ${e.jobNumber}`:''}</span><small>Submitted</small></div><div><button className="primary" onClick={()=>reviewEntry(e,'pm-approve')}>Approve</button><button className="danger" onClick={()=>reviewEntry(e,'reject')}>Reject</button></div></div>)}</section>}<section className="list"><h2>My recent timesheets</h2>{mine.slice(0,10).map(e=><div className="row" key={e.id}><div><b>{e.date} • {e.type}</b><span>{e.totalHours.toFixed(2)} hrs {e.jobNumber?`• Job ${e.jobNumber}`:''}</span><small>{e.status}</small></div>{['Submitted','Rejected'].includes(e.status)&&<div><button onClick={()=>edit(e)}>Edit</button><button className="danger" onClick={()=>del(e)}>Delete</button></div>}</div>)}</section>{user.role==='admin'&&<a className="adminlink" href="/admin">Open Office Admin Console →</a>}</main></div>
}
