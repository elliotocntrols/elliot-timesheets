...
export default function WorkerApp(){
 ...
 const[jobs,setJobs]=useState<JobOption[]>([]),[jobSearch,setJobSearch]=useState(''),[jobOpen,setJobOpen]=useState(false),[jobValid,setJobValid]=useState(false);
 ...
 function choose(j:JobOption){setForm({...form,jobNumber:String(j.id)});setJobSearch(`${j.id} — ${j.name||j.site}`);setJobValid(true);setJobOpen(false)}
 async function save(e:FormEvent){e.preventDefault();if(form.type==='Work'&&!jobValid)return setMessage('Choose a Simpro job.');if(total<=0)return setMessage('Check times.');try{const payload={...form,breakMinutes:0,totalHours:total};if(editing)await api(`/api/timesheets/${editing}`,{method:'PATCH',body:JSON.stringify({action:'edit',...payload})});else await api('/api/timesheets',{method:'POST',body:JSON.stringify(payload)});setMessage(editing?'Updated and resubmitted.':'Submitted.');cancel();await load()}catch(e){setMessage(e instanceof Error?e.message:'Could not save')}}
 function edit(e:Entry){setEditing(e.id);setForm({date:e.date,type:e.type,jobNumber:e.jobNumber,start:e.start,finish:e.finish,breakMinutes:0,notes:e.notes||''});setJobSearch(e.jobNumber?`Job ${e.jobNumber}`:'');setJobValid(!!e.jobNumber);scrollTo({top:0,behavior:'smooth'})}
 function cancel(){setEditing(null);setForm({date:defaultDate,type:'Work',jobNumber:'',start:'07:00',finish:'15:30',breakMinutes:0,notes:''});setJobSearch('');setJobValid(false)}
 ...
 return <div className="worker"><header><div><span>ELLIOT CONTROLS</span><strong>Timesheets</strong></div><button onClick={logout}>Sign out</button></header><main>{message&&<div className="message">{message}</div>}<section className="entry-card"><div className="entry-head"><div><span>{editing?'EDIT ENTRY':'SELECT A DAY'}</span><h1>{selectedDate.toLocaleDateString('en-AU',{weekday:'long',day:'numeric',month:'long'})}</h1></div><div className="hours"><b>{total.toFixed(2)}</b><small>hrs</small></div></div><form onSubmit={save}>
 <div className="types">{workTypes.map(t=><button type="button" key={t} className={form.type===t?'active':''} onClick={()=>{setForm({...form,type:t,jobNumber:t==='Work'?form.jobNumber:''});if(t!=='Work'){setJobSearch('');setJobValid(false)}}}>{t}</button>)}</div>
 {form.type==='Work'&&<label>Job<div className="jobpick"><input value={jobSearch} onFocus={()=>{setJobOpen(true);loadJobs()}} onChange={e=>{setJobSearch(e.target.value);setJobValid(false);setJobOpen(true);loadJobs()}} placeholder="Search job name or number"/>{jobOpen&&<div className="jobmenu">{matches.map(j=><button type="button" key={j.id} onClick={()=>choose(j)}><b>{j.name||`Job ${j.id}`}</b><span>#{j.id}</span></button>)}</div>}</div></label>}
