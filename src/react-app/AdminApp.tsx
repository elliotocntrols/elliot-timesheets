...
export default function AdminApp(){
 ...
 const[editingEntry,setEditingEntry]=useState<Entry|null>(null),[editForm,setEditForm]=useState({date:'',type:'Work',jobNumber:'',start:'07:00',finish:'15:30',notes:''});
 ...
 async function reviewDay(id:string,action:'admin-approve'|'reject'){let reason='';if(action==='reject')reason=prompt('Reason for rejecting this day?')||'Rejected';try{await api(`/api/timesheets/${id}`,{method:'PATCH',body:JSON.stringify({action,reason})});setMessage(action==='admin-approve'?'Day approved for payroll.':'Day rejected.');await load()}catch(e){setMessage(e instanceof Error?e.message:'Could not update day')}}
 function beginAdminEdit(entry:Entry){setEditingEntry(entry);setEditForm({date:entry.date,type:entry.type,jobNumber:entry.jobNumber||'',start:entry.start,finish:entry.finish,notes:entry.notes||''})}
 function adminHours(){if(!editForm.start||!editForm.finish)return 0;const[sh,sm]=editForm.start.split(':').map(Number),[fh,fm]=editForm.finish.split(':').map(Number);let mins=fh*60+fm-(sh*60+sm);if(mins<0)mins+=1440;return Math.max(0,Math.round(mins/60*100)/100)}
 async function saveAdminEdit(){if(!editingEntry)return;const totalHours=adminHours();if(editForm.type==='Work'&&!editForm.jobNumber.trim())return setMessage('Job number is required for worked time.');if(totalHours<=0)return setMessage('Check start and finish times.');try{await api(`/api/timesheets/${editingEntry.id}`,{method:'PATCH',body:JSON.stringify({action:'edit',...editForm,breakMinutes:0,totalHours})});setMessage('Day updated.');setEditingEntry(null);await load()}catch(e){setMessage(e instanceof Error?e.message:'Could not edit day')}}
 ...
 const pendingPeople=Array.from(new Map<string,string>(waiting.map(e=>[e.employeeId,e.employee] as [string,string])).entries()).map(([employeeId,employee])=>{const es=waiting.filter(e=>e.employeeId===employeeId).sort((a,b)=>a.date.localeCompare(b.date));return{employeeId,employee,entries:es,total:es.reduce((a,e)=>a+e.totalHours,0)}});
 const reviewPeople=Array.from(new Map<string,string>(week.entries.map(e=>[e.employeeId,e.employee] as [string,string])).entries()).map(([employeeId,employee])=>{const es=week.entries.filter(e=>e.employeeId===employeeId).sort((a,b)=>a.date.localeCompare(b.date));return{employeeId,employee,entries:es,total:es.reduce((a,e)=>a+e.totalHours,0),pending:es.filter(e=>e.status==='Submitted'||e.status==='PM Approved').length}});
 const approvedPeople=new Set(approved.map(e=>e.employeeId)).size;
 ...
 {tab==='dashboard'&&<><section className="metrics"><Metric l="Active staff" v={active.length}/><Metric l="Approved people" v={approvedPeople}/><Metric l="People awaiting approval" v={pendingPeople.length}/><Metric l="Needs attention" v={new Set(rejected.map(e=>e.employeeId)).size}/></section><section className="panel"><div className="panelhead"><div><span>WEEK REVIEW</span><h2>{reviewPeople.length?`${reviewPeople.length} ${reviewPeople.length===1?'person':'people'}`:'No entries'}</h2><p className="payroll-note">Employees stay grouped by person for the week. Office Admin can edit, approve or reject each day individually.</p></div><button onClick={load}>Refresh</button></div>{reviewPeople.length===0?<p className="muted">No timesheets for this week.</p>:reviewPeople.map(g=>{const open=!!openPeople[g.employeeId];return <div className={`person-approval ${open?'open':''}`} key={g.employeeId}><div className="person-approval-head" role="button" tabIndex={0} aria-expanded={open} onClick={()=>setOpenPeople(v=>({...v,[g.employeeId]:!open}))} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();setOpenPeople(v=>({...v,[g.employeeId]:!open}))}}}><div className="person-approval-toggle"><span className="person-approval-chevron">▶</span><div><b>{g.employee}</b><span>{g.entries.length} entr{g.entries.length===1?'y':'ies'} • {week.label}</span></div></div><span className="person-approval-summary">{g.total.toFixed(2)} hrs • {g.pending} awaiting</span></div><div className="approval-days daily-admin-days">{g.entries.map(e=><div className="admin-day-card" key={e.id}><div className="admin-day-main"><b>{new Date(`${e.date}T00:00:00`).toLocaleDateString('en-AU',{weekday:'short',day:'numeric',month:'short'})}</b><span>{e.type} • {e.totalHours.toFixed(2)} hrs {e.jobNumber?`• Job ${e.jobNumber}`:''}</span><small className={`day-status ${e.status.toLowerCase().replace(/ /g,'-')}`}>{e.status==='Admin Approved'?'Approved for Payroll':e.status}</small></div><div className="admin-day-actions"><button onClick={()=>beginAdminEdit(e)}>Edit</button>{(e.status==='Submitted'||e.status==='PM Approved')&&<><button className="primary" onClick={()=>reviewDay(e.id,'admin-approve')}>Approve</button><button className="danger" onClick={()=>reviewDay(e.id,'reject')}>Reject</button></>}</div></div>)}</div></div>})}</section>{editingEntry&&<div className="admin-edit-backdrop" onClick={()=>setEditingEntry(null)}><div className="admin-edit-modal" onClick={e=>e.stopPropagation()}><div className="admin-edit-head"><div><span>EDIT DAY</span><h2>{editingEntry.employee}</h2></div><button onClick={()=>setEditingEntry(null)}>×</button></div><div className="admin-edit-grid"><label>Date<input type="date" value={editForm.date} onChange={e=>setEditForm({...editForm,date:e.target.value})}/></label><label>Type<select value={editForm.type} onChange={e=>setEditForm({...editForm,type:e.target.value,jobNumber:e.target.value==='Work'?editForm.jobNumber:''})}><option>Work</option><option>Annual Leave</option><option>Sick Leave</option><option>RDO</option><option>Public Holiday</option><option>Training</option><option>Other Paid Leave</option></select></label>{editForm.type==='Work'&&<label className="wide">Job number<input value={editForm.jobNumber} onChange={e=>setEditForm({...editForm,jobNumber:e.target.value})}/></label>}<label>Start<input type="time" value={editForm.start} onChange={e=>setEditForm({...editForm,start:e.target.value})}/></label><label>Finish<input type="time" value={editForm.finish} onChange={e=>setEditForm({...editForm,finish:e.target.value})}/></label><label className="wide">Notes<input value={editForm.notes} onChange={e=>setEditForm({...editForm,notes:e.target.value})}/></label></div><div className="admin-edit-footer"><span>{adminHours().toFixed(2)} hrs</span><div><button onClick={()=>setEditingEntry(null)}>Cancel</button><button className="primary" onClick={saveAdminEdit}>Save day</button></div></div></div></div>}</>}
