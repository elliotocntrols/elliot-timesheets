...
export default function AdminApp(){
 ...
 async function reviewPersonWeek(employeeId:string,action:'approve'|'reject',startDate:string,endDate:string){let reason='';if(action==='reject')reason=prompt('Reason for rejecting this employee week?')||'Rejected';try{const d=await api<{count:number}>('/api/timesheets/week-approval',{method:'PATCH',body:JSON.stringify({employeeId,startDate,endDate,action,reason})});setMessage(action==='approve'?`${d.count} day${d.count===1?'':'s'} approved for payroll.`:`${d.count} day${d.count===1?'':'s'} rejected.`);await load()}catch(e){setMessage(e instanceof Error?e.message:'Could not update employee week')}}
 ...
 {tab==='dashboard'&&<><section className="metrics"><Metric l="Active staff" v={active.length}/><Metric l="Approved people" v={approvedPeople}/><Metric l="People awaiting approval" v={pendingPeople.length}/><Metric l="Needs attention" v={new Set(rejected.map(e=>e.employeeId)).size}/></section><section className="panel"><div className="panelhead"><div><span>WEEK APPROVALS</span><h2>{pendingPeople.length?`${pendingPeople.length} ${pendingPeople.length===1?'person':'people'} waiting`:'All clear'}</h2><p className="payroll-note">Review each person's daily entries, then approve their whole Wednesday–Tuesday week once.</p></div><button onClick={load}>Refresh</button></div>{pendingPeople.length===0?<p className="muted">No employee weeks are waiting for approval.</p>:pendingPeople.map(g=><div className="person-approval" key={g.employeeId}><div className="person-approval-head"><div><b>{g.employee}</b><span>{g.entries.length} entr{g.entries.length===1?'y':'ies'} • {g.total.toFixed(2)} hrs • {week.label}</span></div><div><button className="primary" onClick={()=>reviewPersonWeek(g.employeeId,'approve',week.iso(week.days[0]),week.iso(week.days[6]))}>Approve person</button><button className="danger" onClick={()=>reviewPersonWeek(g.employeeId,'reject',week.iso(week.days[0]),week.iso(week.days[6]))}>Reject person</button></div></div><div className="approval-days">{g.entries.map(e=><div key={e.id}><b>{new Date(`${e.date}T00:00:00`).toLocaleDateString('en-AU',{weekday:'short',day:'numeric',month:'short'})}</b><span>{e.type} • {e.totalHours.toFixed(2)} hrs {e.jobNumber?`• Job ${e.jobNumber}`:''}</span></div>)}</div></div>)}</section></>}
